...
Finds and returns a pointer to the task with the given ID. Returns nullptr if not found.
...
class Task {
public:
    Task(const int _id, const std::string _title): id{_id}, title{_title}, status{Status::PENDING} {}
...
public:
    ...
    [[nodiscard]] std::vector<std::shared_ptr<Task>> get_tasks_with(const Status status) {
        std::vector<std::shared_ptr<Task>> res;
        for(auto& task : tasks) {
            if (task.second->get_status() == status) res.push_back(task.second);
        }
        return res;
    }
    ...
    [[nodiscard]] std::vector<std::shared_ptr<Task>> get_all_tasks() {
        std::vector<std::shared_ptr<Task>> res;
        std::transform(tasks.begin(), tasks.end(), std::back_inserter(res), [](auto& task) { return task.second; });
        return res;
    }

    [[nodiscard]] std::shared_ptr<Task> get_task_by_id(const int id) { 
        for (auto& task : tasks) {
            if (task.second->get_id() == id) return task.second;
        }
        return nullptr;
    }
